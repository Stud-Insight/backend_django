...
from backend_django.core.api import BaseAPI, IsAuthenticated, IsStaff
...
from backend_django.projects.schemas.groups import StagePeriodSchema, TERPeriodSchema
from backend_django.projects.schemas.periods import (
...
class TERPeriodController(BaseAPI):
    ...
    def create_ter_period(self, request: HttpRequest, data: TERPeriodCreateSchema):
        ...
        if not request.user.is_staff:
            return PermissionDeniedError(
                "Seuls les responsables TER peuvent créer des périodes."
            ).to_response()
        ...
        if TERPeriod.objects.filter(
            ...
            return BadRequestError(
                "Une période avec ce nom existe déjà pour cette année académique."
            ).to_response()
    ...
    def update_ter_period(
        ...
        if not request.user.is_staff:
            return PermissionDeniedError(
                "Seuls les responsables TER peuvent modifier des périodes."
            ).to_response()
        ...
        if period.status != PeriodStatus.DRAFT:
            return BadRequestError(
                "Seules les périodes en brouillon peuvent être modifiées."
            ).to_response()
    ...
    def open_ter_period(self, request: HttpRequest, period_id: UUID):
        ...
        if not request.user.is_staff:
            return PermissionDeniedError(
                "Seuls les responsables TER peuvent ouvrir des périodes."
            ).to_response()
        ...
        if period.status != PeriodStatus.DRAFT:
            return BadRequestError(
                f"Impossible d'ouvrir une période avec le statut '{period.status}'."
            ).to_response()
    ...
    def close_ter_period(self, request: HttpRequest, period_id: UUID):
        ...
        if not request.user.is_staff:
            return PermissionDeniedError(
                "Seuls les responsables TER peuvent clôturer des périodes."
            ).to_response()
        ...
        if period.status != PeriodStatus.OPEN:
            return BadRequestError(
                f"Impossible de clôturer une période avec le statut '{period.status}'."
            ).to_response()
    ...
    def copy_ter_period(
        ...
        if not request.user.is_staff:
            return PermissionDeniedError(
                "Seuls les responsables TER peuvent copier des périodes."
            ).to_response()
...
class StagePeriodController(BaseAPI):
    ...
    def create_stage_period(self, request: HttpRequest, data: StagePeriodCreateSchema):
        ...
        if not request.user.is_staff:
            return PermissionDeniedError(
                "Seuls les responsables Stage peuvent créer des périodes."
            ).to_response()
