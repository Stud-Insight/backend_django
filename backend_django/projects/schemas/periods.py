"""
Period schemas for API requests and responses.
"""
...
class TERPeriodCreateSchema(Schema):
    ...
    def name_not_empty(cls, v: str) -> str:
        if not v or not v.strip():
            raise ValueError("Le nom de la période est requis.")
        if len(v.strip()) < 3:
    ...
    def validate_group_size(cls, v: int) -> int:
        if v < 1:
            raise ValueError("La taille du groupe doit être au moins 1.")
        if v > 10:
            raise ValueError("La taille du groupe ne peut pas dépasser 10.")
        return v
    ...
    def validate_dates(self):
        ...
        if self.group_formation_end <= self.group_formation_start:
            errors.append("La fin de formation des groupes doit être après le début.")

        if self.subject_selection_start < self.group_formation_end:
            errors.append("La sélection des sujets doit commencer après la formation des groupes.")

        if self.subject_selection_end <= self.subject_selection_start:
            errors.append("La fin de sélection doit être après le début.")

        if self.assignment_date < self.subject_selection_end:
            errors.append("L'affectation doit être après la fin de sélection.")

        if self.project_start < self.assignment_date:
            errors.append("Le début du projet doit être après l'affectation.")

        if self.project_end <= self.project_start:
            errors.append("La fin du projet doit être après le début.")

        if self.max_group_size < self.min_group_size:
            errors.append("La taille max du groupe doit être >= taille min.")
