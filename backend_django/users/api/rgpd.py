...
class RGPDAdminController(BaseAPI):
    ...
    @http_post(
        "/{user_id}/rgpd/request-deletion",
        response={200: MessageSchema, 401: ErrorSchema, 403: ErrorSchema, 404: ErrorSchema},
        url_name="rgpd_request_deletion",
    )
    def request_deletion(self, request: HttpRequest, user_id: UUID):
        """
        ...
        if str(request.user.id) != str(user_id):
            if not request.user.is_staff:
                return PermissionDeniedError(
                    "Vous ne pouvez demander la suppression que de votre propre compte."
                ).to_response()
        ...
        # 1. Create a DeletionRequest model entry
